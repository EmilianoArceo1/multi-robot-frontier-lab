...
engine.py, no robotics_sim.simulation, no robotics_sim.app import), does not
compute rewards, and never constructs a CriticState or GroundTruthSnapshot --
both only ever arrive as already-built values inside
RuntimeCoordinationCaptureInput, captured by the (future) runtime adapter at
the moment a decision is opened.
...
This service only sequences calls into those four components and classifies
...
Partial-application limitation (see capture_coordination_event): every
...
snapshot key-sets, closing-outcome key-sets, terminal-state/episode_id
agreement) is checked, for every robot in the event, before any component is
mutated -- so those failures never leave an event partially applied. But
episode_session does not expose a pending decision's stored decision_step
through its public API (by design -- see asynchronous_episode.py), so an
outcome whose decision_step is simply wrong for the robot it names can only
be caught once episode_session.complete_robot_decision() itself is called
(the underlying TransitionAssemblyInput raises ValueError). When an event
replaces more than one robot, that call happens once per robot in sequence;
if it fails partway through, the earlier robots in that same event have
already been closed and reopened. Neither InMemoryTrajectoryRecorder nor
InMemoryAsynchronousLearningEpisodeSession expose a public rollback, so this
service cannot undo those earlier robots either -- callers must treat such a
failure as a signal that the caller's own outcome bookkeeping is wrong, not
as a rejected-and-retryable single event.
...
class RuntimeLearningCaptureConsistencyError(RuntimeLearningCaptureError):
    ...
    ASSIGNED/unresolved assignments, or the supplied snapshots was detected
    -- always before any component is mutated for the robot(s) involved."""
...
class RuntimeCoordinationCaptureInput:
    ...
    (future) runtime adapter -- this service never builds any of it.

    ``contexts_by_robot``/``critic_states_by_robot`` are snapshots taken at
    the instant the *new* decision is opened; ``ground_truth_by_robot`` is
    optional and corresponds to that same instant.  ``closing_outcomes_by_robot``
    describes each ASSIGNED robot's *previous* pending decision -- the one
    being closed and replaced by this event, not the one being opened.
    ...
    the plugin, which this dataclass cannot do (see
    RuntimeLearningCaptureService.capture_coordination_event).
    """
    ...
    ground_truth_by_robot: Mapping[int, GroundTruthSnapshot] | None
    ...
    def __post_init__(self) -> None:
        ...
        critic_states_by_robot = dict(self.critic_states_by_robot)
        for robot_id, critic_state in critic_states_by_robot.items():
            _require_robot_id_key("critic_states_by_robot", robot_id)
        ...
        object.__setattr__(self, "critic_states_by_robot", critic_states_by_robot)

        ground_truth_by_robot = (
            None if self.ground_truth_by_robot is None else dict(self.ground_truth_by_robot)
        )
        if ground_truth_by_robot is not None:
            for robot_id, ground_truth in ground_truth_by_robot.items():
                _require_robot_id_key("ground_truth_by_robot", robot_id)
        ...
        object.__setattr__(self, "ground_truth_by_robot", ground_truth_by_robot)
...
class RuntimeLearningCaptureService:
    ...
    def capture_coordination_event(
        ...
        critic_keys = set(capture_input.critic_states_by_robot)
        if critic_keys != assigned_set:
            raise RuntimeLearningCaptureConsistencyError(
                f"critic_states_by_robot keys {sorted(critic_keys)} do not match the ASSIGNED "
                f"robots {sorted(assigned_set)}"
            )
        ground_truth_by_robot = capture_input.ground_truth_by_robot or {}
        extra_ground_truth = set(ground_truth_by_robot) - assigned_set
        if extra_ground_truth:
            raise RuntimeLearningCaptureConsistencyError(
                f"ground_truth_by_robot contains robot id(s) {sorted(extra_ground_truth)} that "
                f"are not ASSIGNED"
            )
        ...
        new_items = tuple(item for item in opened.assigned if item.robot_id in without_pending_set)
        replace_items = tuple(item for item in opened.assigned if item.robot_id in with_pending_set)
        ...
        if new_items:
            ...
            critic_for_new = {
                item.robot_id: capture_input.critic_states_by_robot[item.robot_id]
                for item in new_items
            }
            ground_truth_for_new = {
                item.robot_id: ground_truth_by_robot[item.robot_id]
                for item in new_items
                if item.robot_id in ground_truth_by_robot
            }
        ...
        for item in replace_items:
            # Not atomic across robots: see the module docstring's
            ...
            transition = self._episode_session.complete_robot_decision(
                ...
                next_decision=item,
                next_critic_state=capture_input.critic_states_by_robot[item.robot_id],
                next_ground_truth=ground_truth_by_robot.get(item.robot_id),
            )
            completed_transitions.append(transition)

        return RuntimeCoordinationCaptureResult(
            prepared_decision=prepared,
