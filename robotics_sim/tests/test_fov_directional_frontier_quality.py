"""Quality regressions for the single-robot FoV directional planner.
...
candidate budget is not monopolized by component size, and that routes ending
behind the robot carry an explicit backtracking cost.
"""
...
def test_preselection_balances_size_distance_and_heading_rankings():
    planner = FoVAwareDirectionalFrontierPlanner()
    robot_xy = (0.0, 0.0)
...
def test_scoring_explicitly_penalizes_a_target_behind_the_robot():
    belief = _belief()
    _fill_free(belief)
    robot_xy = belief.cell_to_world((5, 5))
    ahead_cell = (5, 7)
    ...
    behind = _score_candidate(
        candidate=_InternalCandidate(
            behind_cell, belief.cell_to_world(behind_cell), 1, "frontier", (behind_cell,)
        ),
        planning_grid=belief.to_planning_grid(unknown_is_traversable=True),
        **common,
    )
    ...
    assert ahead.score == pytest.approx(0.0)
    assert behind.score == pytest.approx(-1.0)
    assert "backtrack=1.00" in behind.reason
